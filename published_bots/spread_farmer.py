...
MIN_PRINTS          = 20       # need history before quoting
CV_MAX              = 0.08     # skip volatile names (SCA filter)
MIN_SPREAD_BPS      = 40       # worth quoting only if spread > 40 bps
MIN_INSIDE          = 0.02     # absolute minimum improvement on NBBO
EDGE_PCT            = 0.15     # step inside spread by 15% on each side
BASE_SIZE_PCT       = 0.008    # ~0.8% of capital per quote
MAX_INVENTORY_PCT   = 0.02     # per-symbol inventory cap — lowered from 0.03 after account-wide margin pressure
SKEW_STRENGTH       = 0.8      # inventory skew into quote price
